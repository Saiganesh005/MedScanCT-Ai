...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train FastViT on COVIDxCT using preprocessed images and CSV splits."
    )
    parser.add_argument(
        "--data-root",
        type=Path,
...
def create_eval_transforms(image_size: int) -> transforms.Compose:
    return transforms.Compose(
        [
            transforms.Resize((image_size, image_size)),
            transforms.ToTensor(),
...
def create_dataloaders(
    ...
    train_transform = create_train_transforms(args.image_size)
    eval_transform = create_eval_transforms(args.image_size)

    train_dataset = CovidXCTDataset(train_samples, label_map, train_transform)
    val_dataset = CovidXCTDataset(val_samples, label_map, eval_transform)
    test_dataset = CovidXCTDataset(test_samples, label_map, eval_transform)
